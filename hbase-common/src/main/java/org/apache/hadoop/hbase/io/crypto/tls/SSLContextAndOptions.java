...
public class SSLContextAndOptions {
  ...
  SSLContextAndOptions(final X509Util x509Util,
    final Configuration config, final SSLContext sslContext) {
  ...
  private String[] getEnabledProtocols(final Configuration config,
    final SSLContext sslContext) {
    String enabledProtocolsInput = config.get("hbase.security.tls.enabledProtocols");
  ...
  private String[] getCipherSuites(final Configuration config) {
    String cipherSuitesInput = config.get("hbase.security.tls.ciphersuites");
  ...
  private int getHandshakeDetectionTimeoutMillis(final Configuration config) {
    String propertyString = config.get("hbase.security.tls.handshakeDetectionTimeoutMillis");
    int result;
    if (propertyString == null) {
      result = org.apache.zookeeper.common.X509Util.DEFAULT_HANDSHAKE_DETECTION_TIMEOUT_MILLIS;
    } else {
      ...
      if (result < 1) {
        ...
        LOG.warn(
          ...
          "hbase.security.tls.handshakeDetectionTimeoutMillis",
          result,
          org.apache.zookeeper.common.X509Util.DEFAULT_HANDSHAKE_DETECTION_TIMEOUT_MILLIS);
