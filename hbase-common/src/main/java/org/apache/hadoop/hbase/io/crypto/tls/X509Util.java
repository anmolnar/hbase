 ...
 *
 *   Performance testing done by Facebook engineers shows that on Intel x86_64 machines,
 *   Java9 performs better with GCM and Java8 performs better with CBC, so these seem like
 *   reasonable defaults.
 */
@InterfaceAudience.Private
public class X509Util implements Closeable, AutoCloseable {

  private static final Logger LOG =
    LoggerFactory.getLogger(X509Util.class);

  // Config
  private static final String CONFIG_PREFIX = "hbase.security.tls.";
  private static final String TLS_CONFIG_PROTOCOL = CONFIG_PREFIX + "protocol";
  private static final String TLS_CONFIG_KEYSTORE_LOCATION = CONFIG_PREFIX + "keystore.location";
  private static final String TLS_CONFIG_KEYSTORE_TYPE = CONFIG_PREFIX + "keystore.type";
  private static final String TLS_CONFIG_KEYSTORE_PASSWORD = CONFIG_PREFIX + "keystore.password";
  private static final String TLS_CONFIG_TRUSTSTORE_LOCATION =
    CONFIG_PREFIX + "truststore.location";
  private static final String TLS_CONFIG_TRUSTSTORE_TYPE = CONFIG_PREFIX + "truststore.type";
  private static final String TLS_CONFIG_TRUSTSTORE_PASSWORD =
    CONFIG_PREFIX + "truststore.password";
  private static final String TLS_CONFIG_CLR = CONFIG_PREFIX + "clr";
  private static final String TLS_CONFIG_OCSP = CONFIG_PREFIX + "ocsp";
  ...
  private static final String[] DEFAULT_CIPHERS_JAVA8 = concatArrays(getCBCCiphers(),
    getGCMCiphers());
  ...
  private static final String[] DEFAULT_CIPHERS_JAVA9 = concatArrays(getGCMCiphers(),
    getCBCCiphers());
    ...
    if (keyStoreLocation.isEmpty()) {
      ...
      try {
        keyManagers = new KeyManager[] {
          createKeyManager(keyStoreLocation, keyStorePassword, keyStoreType)
        };
    ...
    if (trustStoreLocation.isEmpty()) {
      ...
      try {
        trustManagers = new TrustManager[]{
          createTrustManager(trustStoreLocation, trustStorePassword, trustStoreType,
  ...
  public X509KeyManager createKeyManager(String keyStoreLocation,
    ...
    try {
      ...
      try (InputStream inputStream = new BufferedInputStream(
        Files.newInputStream(new File(keyStoreLocation).toPath()))) {
  ...
  public static X509TrustManager createTrustManager(
    ...
    try {
      ...
      try (InputStream inputStream = new BufferedInputStream(
        Files.newInputStream(new File(trustStoreLocation).toPath()))) {
